...
// Types - using Figma's built-in Annotation type
...
async function collectAnnotations(root: SceneNode): Promise<CollectedAnnotation[]> {
  const results: CollectedAnnotation[] = [];

  function traverse(node: SceneNode) {
    // Check if node has annotations (Dev Mode feature)
    if ('annotations' in node) {
      const nodeWithAnnotations = node as SceneNode & { annotations?: readonly Annotation[] };
...
async function generateCalloutsForFrame(
  annotations: CollectedAnnotation[],
  frameBounds: { x: number; y: number; width: number; height: number }
): Promise<SceneNode[]> {
  const elementsCreated: SceneNode[] = [];
...
async function generateCallouts(frames: SceneNode[]): Promise<number> {
  // Load required fonts
...
function removeCallouts(): number {
  const page = figma.currentPage;
  const calloutGroups = page.findAll(
    (n: SceneNode) => n.name === CALLOUT_GROUP_NAME
  );
  ...
  if (msg.type === 'generate') {
    try {
      ...
      if (selection.length > 0) {
        ...
        targets = selection.filter(
          (n) => n.type === 'FRAME' || n.type === 'GROUP' || n.type === 'COMPONENT' || n.type === 'INSTANCE'
        );
        ...
        targets = figma.currentPage.children.filter(
          (n: SceneNode) => n.type === 'FRAME' && n.name !== CALLOUT_GROUP_NAME
        );
      }
